...
# Probes: the canonical 0x000E device-info poke plus a channel-status read.
...
class DiscoveredGateway:
    ...
    def label(self) -> str:
        ...
        extra = (
            f" · {len(self.bus_addresses)} bus device(s) heard"
            if self.bus_addresses
            else ""
        )
        return f"{self.ip} — HDL Buspro gateway{extra}"
...
def _make_probe_socket() -> socket.socket:
    ...
    reply lands on this socket -- even when a running buspro entry already
    holds 0.0.0.0:6000.
    """
...
def _make_listen_socket() -> socket.socket | None:
    ...
    Returns None if the port cannot be bound (non-fatal; the probe socket
    is always used and is sufficient for most cases).
    """
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
async def async_discover_gateways(
    ...
    Returns gateways sorted by frame count (busiest first), so the most
    likely "real" gateway is first in the list.
    """
    ...
    found = sorted(shared_found.values(), key=lambda g: (-g.frames, g.ip))
    _LOGGER.info(
