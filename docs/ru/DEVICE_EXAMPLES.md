...
**Содержание:**
- [Подтверждение Команды (НОВОЕ!)](#подтверждение-команды-новое)
...
## Подтверждение Команды (НОВОЕ!)

### Что такое Подтверждение Команды?

Подтверждение команды гарантирует, что изменения состояния устройства отражаются в Home Assistant только после физического подтверждения устройством получения и выполнения команды. Это предотвращает рассинхронизацию интерфейса, когда команды теряются из-за помех в сети.

**Без Подтверждения:**
- Пользователь нажимает "Включить"
- Интерфейс обновляется сразу же (~5ms)
- Устройство получает команду примерно через ~100ms
- Если устройство не получит → Интерфейс показывает неправильное состояние

**С Подтверждением:**
- Пользователь нажимает "Включить"
- Система ожидает подтверждения устройства (~100-500ms)
- Устройство подтверждает получение и выполнение
- Интерфейс обновляется только после подтверждения
- Если устройство не реагирует → Явная ошибка истечения времени ожидания

### Зачем это нужно

Включите подтверждение для:
- **Критических устройств** - Реле аварийной остановки, главные выключатели
- **Ненадежных сетей** - Высокие помехи, много коллизий
- **Зависимостей автоматизации** - Автоматизация, требующая гарантированного состояния
- **Устройств, критических для безопасности** - Системы HVAC, тепловые полы

### Примеры Конфигурации

#### Для Критического Реле
...
light:
  - platform: buspro
    devices:
      "1.10.1":
        name: "Реле аварийной остановки"
        enable_confirmation: true
        confirmation_timeout: 5.0
        confirmation_retries: 3
```

#### Для Нескольких Критических Устройств
...
light:
  - platform: buspro
    devices:
      # Критическое - подтвердить получение
      "1.10.1":
        name: "Основной потолочный свет"
        enable_confirmation: true
      
      # Некритическое - держать быстро (по умолчанию)
      "1.10.2":
        name: "Фоновое освещение"
        # enable_confirmation по умолчанию false
  ...
  - platform: buspro
    devices:
      "2.10.1":
        name: "Спальные шторы"
        enable_confirmation: true
        confirmation_timeout: 10.0  # Больше для механических устройств
...
| Параметр | Тип | По умолчанию | Диапазон | Назначение |
|-----------|------|---------|-------|---------| 
| `enable_confirmation` | boolean | `false` | `true`/`false` | Включить/отключить подтверждение |
| `confirmation_timeout` | float | `5.0` | 0.1-60 секунд | Макс. время ожидания ответа устройства |
| `confirmation_retries` | integer | `3` | 0-10 | Попытки повтора при истечении времени |

### Рекомендации по Типам Устройств

| Тип Устройства | Время Ожидания | Повторы | Примечания |
|------------|---------|---------|-------|
| Реле/Выключатель | 5.0s | 3 | Быстрое электронное устройство |
| Свет/Диммер | 5.0s | 3 | Быстрое электронное устройство |
| Вентилятор | 5.0s | 3 | Быстрое электронное устройство |
| Жалюзи/Штора | 10.0s | 2 | Механическое устройство, медленнее |
| Климат AC | 5.0s | 3 | Электронное устройство |
| Тепловой пол | 5.0s | 3 | Электронное устройство |

### Влияние на Пользовательский Интерфейс

Когда подтверждение включено:
- **Задержка:** Отсрочка 100-500ms (против 5-10ms без)
- **Обратная связь:** Четкое указание успеха/отказа
- **Надежность:** Гарантированная синхронизация состояния

Когда подтверждение отключено (по умолчанию):
- **Задержка:** ~5-10ms (без изменений)
- **Поведение:** Fire-and-forget (текущее поведение)
- **Риск:** Возможны молчаливые сбои команд
