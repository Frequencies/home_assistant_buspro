...
class TelegramHelper:
    ...
    def __init__(self, client_address, advertised_ip):
    ...
    def replace_none_values(self, telegram: Telegram):
        if telegram is None:
            return None
        if telegram.payload is None:
            telegram.payload = []
        if telegram.source_address is None:
            telegram.source_address = list(self._client_address)
    ...
    def build_send_buffer(self, telegram: Telegram):
        try:
            source_ip = socket.inet_aton(self._advertised_ip or "0.0.0.0")
        except OSError as err:
            raise ValueError(
                f"Invalid Buspro advertised IP: {self._advertised_ip!r}"
            ) from err
        send_buf = bytearray(source_ip)
        # noinspection SpellCheckingInspection
        ...
        if telegram.source_address is not None:
            ...
            sender_subnet_id, sender_device_id = self._client_address
