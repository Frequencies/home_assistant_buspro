...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    def _expected_floor_module_channels(address):
        registered = device_registry.async_get_device(
            identifiers={(DOMAIN, address)}, connections=set()
        )
        if registered is None or not registered.model:
            return None
        spec = DEVICE_CATALOG.get(registered.model, {})
        if spec.get(CONF_DEVICE_TYPE) != DEVICE_TYPE_FLOOR_HEATING:
            return None
    ...
    for device_config in config[CONF_DEVICES]:
        ...
        if fh_type == FloorHeatingDeviceType.Module:
            module_state = module_channels.setdefault(
                address,
                {
                    "device_address": device_address,
                    "channels": set(),
                    "name": name,
                    "expected": _expected_floor_module_channels(address),
                },
            )
    ...
    for address, state in module_channels.items():
        expected_count = state["expected"]
        if expected_count is None:
            continue
        configured_channels = state["channels"]
        for channel_number in range(1, expected_count + 1):
            if channel_number in configured_channels:
                continue
            device = FloorHeating(
                hdl,
                state["device_address"],
                f"{state['name']} Channel {channel_number}",
                channel_number=channel_number,
                device_type=FloorHeatingDeviceType.Module,
            )
            devices.append(
                BusproFloorHeatingClimate(
                    hass,
                    device,
                    [PRESET_NONE, PRESET_AWAY, PRESET_HOME, PRESET_SLEEP],
                    None,
                    f"hdl_buspro_floor_heating_{address.replace('.', '_')}_{channel_number}",
                    f"{DOMAIN}-{address}-floor_heating-{channel_number}",
                    None,
                    None,
                    None,
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    """Set up UI-managed AC and floor-heating entities."""
    from .pybuspro.devices import Climate, FloorHeating
    ...
    for device_config in config_entry.options.get(CONF_MANAGED_DEVICES, []):
        if device_config[CONF_DEVICE_TYPE] == DEVICE_TYPE_AC:
            address = tuple(
                int(part) for part in device_config["address"].split(".")
            )
            info = managed_device_info(device_config)
            channel = device_config[CONF_CHANNELS][0]
            device = Climate(module.hdl, address, channel[CONF_NAME])
            entities.append(
                BusproACClimate(
                    hass,
                    device,
            ...
            continue
        if device_config[CONF_DEVICE_TYPE] != DEVICE_TYPE_FLOOR_HEATING:
            continue
        address = tuple(int(part) for part in device_config["address"].split("."))
        info = managed_device_info(device_config)
        channels = device_config[CONF_CHANNELS]
        model_spec = DEVICE_CATALOG.get(device_config.get(CONF_MODEL), {})
        expected_channel_count = max(
            len(channels),
            int(model_spec.get("channels", 0)),
            managed_channel_count(device_config),
        )
        if len(channels) < expected_channel_count:
            ...
            channel_keys = list(range(1, expected_channel_count + 1))
            names = {
                key: existing_channels.get(key, {}).get(CONF_NAME, "")
                for key in channel_keys
            }
            channels = build_managed_channels(
                device_config["address"],
                device_config[CONF_DEVICE_TYPE],
        ...
        for channel in channels:
            ...
            device = FloorHeating(
                ...
                address,
                channel[CONF_NAME]
                or f"Channel {channel[CONF_CHANNEL_NUMBER]}",
